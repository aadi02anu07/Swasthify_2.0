...
function VitalCard({ config, reading, index }) {
  ...
  if (config.key === 'bp') {
    displayValue = (reading?.bpSystolic != null && reading?.bpDiastolic != null)
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {vitalsConfig.map((config, i) => (
          <VitalCard key={config.key} config={config} reading={null} index={i} />
        ))}
      </div>
    )
  }
    ...
    <div>
      {reading?.recordedAt && (
        <p className="text-xs text-slate-500 mb-3">
          ...
          {reading.recordedBy?.name && <span> · {reading.recordedBy.name}</span>}
          {reading.hospital?.name && <span className="text-blue-400/80"> · {reading.hospital.name}</span>}
        </p>
      )}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {vitalsConfig.map((config, i) => (
      ...
      {reading?.notes && (
        <div className="mt-3 p-3 rounded-xl text-sm text-slate-400"
          style={{ background: 'rgba(30,58,95,0.2)', border: '1px solid rgba(30,58,95,0.3)' }}>
